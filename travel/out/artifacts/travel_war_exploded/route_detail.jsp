...
<script>
    ...
    $(function () {
        ...
        $.post("route/findRouteByRid.do",{"rid": rid},function (route) {
            // 导航条面包屑
            var html = "";
            var category = route.category;
            html += " <div class='bread_box'>\n" +
                "                    <a href='index.jsp'>首页</a>\n" +
                "                    <span> &gt;</span>\n" +
                "                    <a href='/route_list.jsp?cid=" + category.cid+ "'>"+ category.cname+"</a><span> &gt;</span>\n" +
                "                <a href=''>" + route.rname +"</a>\n" +
                "                </div>";

            $(".wrap .bread_box").html(html);
            //线路详细信息
            ...
            //更新收藏次数
            $(".collect span:eq(1)").html("已收藏" + route.count +"次");

            //图片更新
            var routeImages = route.routeImgList;


            //上部
            html = " <dt>\n" +
                "     <img alt='' class='big_img' src='" + routeImages[0].bigPic+  "'>\n" +
                "     </dt>";

            //轮播图
            html += "  <dd > <a class=\"up_img up_img_disable\"></a>";
            for(var img of routeImages){
        ...
        //检测是否已经收藏
        $.post("route/isFavoriteByRid.do",{"rid" : rid } ,function (data) {

            if(data){
                $(".collect span:eq(0)").html("<a  class=\"btn already\" disabled=\"disabled\"><i class=\"glyphicon glyphicon-heart-empty\"></i>点击收藏</a>\n")
            }else{
                $(".collect span:eq(0)").html("<a class=\"btn\"><i class=\"glyphicon glyphicon-heart-empty\"></i>点击收藏</a>\n")
                //给点击收藏按钮绑定事件
                console.log(123);
                $(".collect span .btn").click(function () {
                    //发送请求 添加favorite 如果没有登录 就跳转到登录页面 如果已经登录 就开始添加
                    $.post("favorite/addFavorite.do",{"rid" : rid} ,function (data) {
                        ...
                        if(data == false){
                            ...
                            //修改收藏的数量
                            $(".collect span:eq(1)").html("已收藏" + data +"次");
